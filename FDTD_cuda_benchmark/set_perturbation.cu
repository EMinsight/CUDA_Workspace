...
void set_perturbation( perturbation P_info, float* noise_Nh, float* Nh){

    Eigen::Vector3d R_c;
    Eigen::Vector3d R;
    Eigen::Vector3d R_d;

    float z_0{ dist(P_info.r0()) };
    float R_theta{ 0.0 };
    float d_h{ 0.0 };
    ...
    for( int i = 0; i < ion_L; i++ ){
        ...
        for( int j = 0; j < Ntheta; j++ ){
            for( int k = 0; k < Nphi; k++ ){
                ...
                float = enhance = P_info.alpha()*std::exp(- (std::pow(d_h, 2.0)/2.0/std::pow(P_info.sig_h(), 2.0)))
                                                *std::exp(- (std::pow(z - z_0, 2.0)/2.0/std::pow(P_info.sig_r(), 2.0)));
                
                noise_Nh[i*(Ntheta * Nphi) + j*Nphi + k] = Nh[i] + Nh[i] * enhance;
